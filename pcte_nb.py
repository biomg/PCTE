    ...
    while count < n:
        ...
        with torch.no_grad():
            ...
            for data in dataloader_train_12:
                    ...
                    input = input.cuda()
                    label = label.cuda()
                    out = model.getfeature(input)
                    all_feature.append(out.cpu().numpy())
        ...
        with torch.no_grad():
                ...
                for data in dataloader_test_12:
                    ...
                    label = label.cuda()
                    out = model.getfeature(input)
                    test_feature.append(out.cpu().numpy())
        ...
        rf_model.fit(all_feature, all_label)
        rf_probs = rf_model.predict_proba(test_feature)[:, 1]
        auc_score = roc_auc_score(test_label, rf_probs)
        print(f"AUC: {auc_score}")
        auprc = average_precision_score(test_label, rf_probs)
